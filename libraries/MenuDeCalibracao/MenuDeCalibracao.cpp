...
const long  IR2_0    = 0xFF6897;
const long  IR2_1    = 0xFF30CF;
const long  IR2_2    = 0xFF18E7;
const long  IR2_3    = 0xFF7A85;
const long  IR2_4    = 0xFF10EF;
const long  IR2_5    = 0xFF38C7;
const long  IR2_6    = 0xFF5AA5;
const long  IR2_7    = 0xFF42BD;
const long  IR2_8    = 0xFF4AB5;
const long  IR2_9    = 0xFF52AD;
const long  IR2_OK   = 0xFFE21D;
const long  IR2_CIMA   = 0xFFA857;
const long  IR2_BAIXO  = 0xFF906F;
const long  IR2_ESQUERDA = 0xFF02FD;
const long  IR2_DIREITA  = 0xFFC23D;
const long  IR2_LIGA   = 0xFFA25D;
const long  IR2_MENOS  = 0xFF9867;
const long  IR2_PLAY   = 0xFF22DD;
const long  IR2_PAUSE  = 0xFFB04F;
const long  IR2_STOP   = 0xFFE01F;
const long  IR2_VOLTA  = 0xFF629D;

bool funcionamentoSemCalibracao;
bool LCDdesligado;
long ultimoTempoComBotaoApertado;
...
MenuBackend menu = MenuBackend(MenuDeCalibracao::menuUseEvent,MenuDeCalibracao::menuChangeEvent);
...
using namespace variaveisLCD;

//this function builds the menu and connects the correct items together
void MenuDeCalibracao::menuSetup(){
...
*/
void MenuDeCalibracao::menuUseEvent(MenuUseEvent used){
 ...
 // Serial.println(used.item.getName());
  ...
  if(used.item.getName()==MENU_INICIALOG_MODOEXIB){
    setModoApenasLeitura();
    lcd.clear();
    lcd.print("Iniciou SD");
    delay(500);
    menu.moveLeft();
  }

  if(used.item.getName()==MENU_INICIALOG_MODOSEMLCD){
    ligaDesligaTela();
  }
...
*/
void MenuDeCalibracao::menuChangeEvent(MenuChangeEvent changed){
...
void MenuDeCalibracao::pwmConfiguraSelecionado(int selecionado){
  if(!taNaTelaCalibPWM){
      bitmap.clear();
      lcd.clear();
...
void MenuDeCalibracao::pwmConfigCancela(){
      calibNum1 = -1;
      calibNum2 = -1;
      taNoModoInput = false;
...
void MenuDeCalibracao::mostraMenuEscondido(){
      menu.moveDown();
      menu.moveUp();
}

void MenuDeCalibracao::iniciaMenuDeCalibracao(int numDadosExibidos)
{
 // Serial.begin(9600);
  lcd.begin(16, 2);
  bitmap.begin();
  startScreen();
  menuSetup();
  numTotalDados = numDadosExibidos;
  bitmap.clear();
  menu.moveDown();
...
void MenuDeCalibracao::atualizaMenuDados(){
//  String oi[5] = {"ESSA", "MERDA", "NAO", "FUNCIONA", "QUERO DORMIR"};
//  setDadosParaExibir(dadosVindoParaExibir);
//  setValorUltrassomRaw(7);
//  setValorPWMRaw(1300, 1500, 1400, 1500);

  verificaControleIR();
  if(millis() - ultimoTempoComBotaoApertado > 60000){
    if(lcdLigado()){
...
void MenuDeCalibracao::setValorUltrassomRaw(int dist){
    if(taNaTelaCalibUltra){
      printaTelinhaCalibrUltrassom(dist);
    }
}

void MenuDeCalibracao::setValorPWMRaw(int PWM1, int PWM2, int PWM3, int PWM4){
    if(taNaTelaCalibPWM){
      switch (pwmExibido) {
          case 1:
...
void MenuDeCalibracao::setTela(String msg){
  if(lcdLigado()){
      lcd.clear();
      lcd.print(msg);
  }
...
void MenuDeCalibracao::setDadosParaExibir(String dadosVindo[]){
  if(lcdLigado()){
      if(taNaTelaDeDados){
...
void MenuDeCalibracao::printaTelinhaCalibrPWM(int valor){
  lcd.clear();
  bitmap.clear();
  if(taNoPWMZero){
    bitmap.move(0,0);
...
void MenuDeCalibracao::printaTelinhaCalibrUltrassom(int valor){
  lcd.clear();
  bitmap.move(0,0);
  if((valor < 16)&&(valor >= 0)){
...
void MenuDeCalibracao::printaTelinhaDados(String dado){
    char buff[50];
...
void MenuDeCalibracao::printaTelinha(char linha1[], char linha2[], bool selecaoLinha0, bool mostraEsquerda,  bool clicavel, bool mostraDireita){
    lcd.clear();
    if(selecaoLinha0){
      if(mostraEsquerda){
        lcd.print("<");
...
void MenuDeCalibracao::trataNumero(int n){
...
void MenuDeCalibracao::startScreen(){
   byte x = 9;
...
void MenuDeCalibracao::verificaControleIR(){
  if (irrecv.decode(&results))  {  
    ultimoTempoComBotaoApertado = millis();
...
//          Serial.println("8"); 
  ...
  void MenuDeCalibracao::setModoApenasLeitura(){
    funcionamentoSemCalibracao = true;
  }

  void MenuDeCalibracao::ligaDesligaTela(){
    funcionamentoSemCalibracao = true;
    if(LCDdesligado == true){
          LCDdesligado = false;
          lcd.display();
    }else{
          LCDdesligado = true;      
  ...
  bool MenuDeCalibracao::emModoLeitura(){
    return funcionamentoSemCalibracao;
  }

  bool MenuDeCalibracao::lcdLigado(){
    return !LCDdesligado;
  }
