...
// ---------------------------------------------------------------------------
// NewPing Start --- Método criado posteriormente para colocação de pinos
// ---------------------------------------------------------------------------

void NewPingCA::start(uint8_t trigger_pin, uint8_t echo_pin, int max_cm_distance) {
...
unsigned int NewPingCA::ping_in() {
	unsigned int echoTime = NewPingCA::ping();          // Calls the ping method and returns with the ping echo distance in uS.
	return NewPingConvert(echoTime, US_ROUNDTRIP_IN); // Convert uS to inches.
}


unsigned int NewPingCA::ping_cm() {
	unsigned int echoTime = NewPingCA::ping();          // Calls the ping method and returns with the ping echo distance in uS.
	return NewPingConvert(echoTime, US_ROUNDTRIP_CM); // Convert uS to centimeters.
}


unsigned int NewPingCA::ping_median(uint8_t it) {
...
boolean NewPingCA::ping_trigger() {
#if DISABLE_ONE_PIN != true
	*_triggerMode |= _triggerBit;    // Set trigger pin to output.
#endif
...
void NewPingCA::ping_timer(void (*userFunc)(void)) {
	if (!ping_trigger()) return;         // Trigger a ping, if it returns false, return without starting the echo timer.
	timer_us(ECHO_TIMER_FREQ, userFunc); // Set ping echo timer check every ECHO_TIMER_FREQ uS.
}

 
boolean NewPingCA::check_timer() {
...
void NewPingCA::timer_us(unsigned int frequency, void (*userFunc)(void)) {
...
void NewPingCA::timer_ms(unsigned long frequency, void (*userFunc)(void)) {
	timer_setup();                       // Configure the timer interrupt.
	intFunc = NewPingCA::timer_ms_cntdwn;  // Timer events are sent here once every ms till user's frequency is reached.
...
void NewPingCA::timer_stop() { // Disable timer interrupt.
#if defined (__AVR_ATmega32U4__) // Use Timer4 for ATmega32U4 (Teensy/Leonardo).
	TIMSK4 = 0;
#else
	TIMSK2 &= ~(1<<OCIE2A);
...
void NewPingCA::timer_ms_cntdwn() {
...
unsigned int NewPingCA::convert_in(unsigned int echoTime) {
	return NewPingConvert(echoTime, US_ROUNDTRIP_IN); // Convert uS to inches.
}


unsigned int NewPingCA::convert_cm(unsigned int echoTime) {
	return NewPingConvert(echoTime, US_ROUNDTRIP_CM); // Convert uS to centimeters.
}
